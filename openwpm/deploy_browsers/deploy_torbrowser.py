...
import logging
import os
#TFS: import environment variables.
import tempfile
...
from ..utilities.platform_utils import get_torbrowser_binary_path, get_torbrowser_profile_path
...
def deploy_torbrowser(
    ...
    """
    launches a TB instance with parameters set by the input dictionary
    """
    os.environ['TOR_SKIP_LAUNCH'] = '1'
    os.environ['TOR_TRANSPROXY'] = '1'
    torbrowser_binary_path = get_torbrowser_binary_path()

    root_dir = os.path.dirname(__file__)  # directory of this file

    # TFS: Replace this code to just grab the default Tor profile(s)?
    #browser_profile_path = Path(tempfile.mkdtemp(prefix="firefox_profile_"))
    browser_profile_path = Path(get_torbrowser_profile_path(browser_params.slider_level))
    status_queue.put(("STATUS", "Profile Grabbed", browser_profile_path))
    ...
    fb = FirefoxBinary(firefox_path=torbrowser_binary_path)
